...
fn tool_def_search(registry: &ProjectRegistry) -> Value {
    let mut def = json!({
        ...
        "description": "CRITICAL WORKFLOW: Search BEFORE investigating unfamiliar code or architecture in this repository to check if there are relevant decisions, context, or bug investigations already documented (skip when exact file/line is given, for mechanical tasks, or when context is already known). Search the project's knowledge base for design decisions, architecture notes, feature specs, bug investigation records, and other institutional knowledge. Supports full-text search and keyword-based search. Returns matching entries with relevance scores. A result's `project` field is the repo it was saved from (`owner/repo`), which is how a user-scope hit says where it came from — not the project this request targeted.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
...
fn call_tool(name: &str, params: &Value, registry: &ProjectRegistry) -> Result<Value, String> {
    ...
    match name {
        "search_knowledge" => {
            ...
            let is_mine = |label: &str, e: &db::Entry| {
                label == "project" || (here.is_some() && e.project.as_deref() == here.as_deref())
            };
            items.sort_by(|a, b| {
                a.0.partial_cmp(&b.0)
