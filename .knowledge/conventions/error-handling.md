...
Most commands support a `--json` flag for machine-readable output. JSON is constructed using `serde_json::json!()` macro and printed with `serde_json::to_string_pretty()`. Human-readable output uses simple `println!` formatting.
