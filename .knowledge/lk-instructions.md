...
- **Plans you design** (plan-mode or in conversation) → **save proactively, without asking**, via `/lk-knowledge-plan` as `category: plan` + `status: proposed`, once out of plan mode. Persists across compaction/session loss. Details, density bar & lifecycle: `/lk-knowledge-plan`.
