...
- Only a **same-title** collision refuses the add (`added: false` + `similar_entries`) — update that entry instead, or pass `--force` / `force: true` to add a second entry under that title. Otherwise the add succeeds and any loosely related entries come back under `possibly_related`, which is **informational only**: the entry is already saved, so do not update one of those unless it genuinely covers the same topic.
