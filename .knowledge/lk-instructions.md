...
- **Plans you design** (a plan-mode plan after `ExitPlanMode`, or an approach worked out in plain conversation) → **save it proactively, without asking first**, as `category: plan` + `status: proposed` + keyword `plan`, once the plan is settled (and tools are available again — i.e. after leaving plan mode). Save **every** plan you design, not just deferred ones: it persists across compaction/session loss so the work is instantly recoverable from lk later. Write it **dense and self-contained** (decision, approach, rejected options, dead-ends, concrete identifiers) — same density bar as context. Briefly note the id/uid. Lifecycle: `proposed` (open) → `accepted` (done, `lk edit <id> --status accepted`) → `deprecated` (dropped). Manual hub (save / list-and-resume / done / drop): `/lk-knowledge-plan`.
