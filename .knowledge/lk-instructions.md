...
- **Scope**: project (default) vs user (global `~/.config/lk/knowledge.db`, shared across all projects). Use `--scope user` / `scope: "user"` for personal prefs or cross-project context not tied to this repo. Reads merge both by default. Address user entries by uid (numeric ids are project-only).
