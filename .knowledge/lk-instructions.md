...
- Content containing potential secrets is blocked (`added: false` + `secret_detected: true`) — strip the secret, or pass `--allow-secrets` / `allow_secrets: true` if it is a false positive. Only a **same-title** collision (or an all-but-identical title) refuses on duplication (`added: false` + `similar_entries`) — edit that entry instead (`lk edit` / `edit_knowledge`), or pass `--force` / `force: true` to add it anyway. A non-duplicate entry without secrets is saved (`added: true`), and any loosely related entries come back under `possibly_related`, which is **informational only** — the entry is already saved, so do not edit one of those unless it genuinely covers the same topic.
...
- **Project attribution**: every entry records the project it was added from (the git remote slug `owner/repo`, or a directory name when there is no remote), so a user-scope hit says where it came from even in a project without `lk init`. It shows as `@repo-name` in search/list output (only when it differs from the current project) and as `project` in `--json` / `lk get`. In MCP, `add_knowledge` defaults to the target project resolved for the request (the startup directory in single-project mode) — **pass `recorded_project` whenever saving findings about a different repo** (another checkout, worktree, or submodule). Correct a misattributed entry with `edit_knowledge(id, recorded_project: "owner/repo")` (or CLI `lk edit <id> --project <owner/repo>`, `--project ""` / `recorded_project: ""` to clear). On the CLI, override per add with `lk add --project <owner/repo>` or `LK_PROJECT`; `git config lk.project <owner/repo>` sets it for a repo for good (honored through MCP too). Filter by it with `search`/`list` `--project` (`owner/repo` exact, a bare repo name for any owner, `.` for the current project) or the MCP `project_filter` parameter — useful to pull knowledge saved while working on another repo.
