...
- **Project attribution**: every entry records the project it was added from (the git remote slug `owner/repo`, or a directory name when there is no remote), so a user-scope hit says where it came from even in a project without `lk init`. It shows as `@repo-name` in search/list output (only when it differs from the current project) and as `project` in `--json` / `lk get`. Override per add with `lk add --project <owner/repo>` or `LK_PROJECT`; `git config lk.project <owner/repo>` sets it for a repo for good (honored through MCP too). Filter by it with `search`/`list` `--project` (`owner/repo` exact, a bare repo name for any owner, `.` for the current project) or the MCP `project_filter` parameter — useful to pull knowledge saved while working on another repo. `lk edit <id> --project` backfills an entry, `--project ""` clears it.
