...
- **Project attribution**: every entry records the project it was added from (the git remote slug `owner/repo`, or a directory name when there is no remote), so a user-scope hit says where it came from even in a project without `lk init`. It shows as `@repo-name` in search/list output (only when it differs from the current project) and as `project` in `--json` / `lk get`. Override with `lk add --project <owner/repo>` or `LK_PROJECT`.
