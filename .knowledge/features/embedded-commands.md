...
Seven Claude Code slash commands are compiled into the binary via `include_str!()` macro in the `EMBEDDED_COMMANDS` constant in `src/main.rs`. The `install_embedded_commands()` function writes them to `~/.claude/commands/`. Commands are: `~lk-knowledge-search.md` (search), `~lk-knowledge-add.md` (add knowledge), `~lk-knowledge-export.md` (export), `~lk-knowledge-sync.md` (sync), `~lk-knowledge-write.md` (write helper), `~lk-knowledge-discover.md` (project auto-scan), and `~lk-knowledge-refresh.md` (check and update stale entries). Embedding in the binary provides MITM protection over network-fetched commands.
