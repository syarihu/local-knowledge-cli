...
The `.github/workflows/release.yml` (86 lines) triggers on `v*` tags and has 2 jobs. The `build` job uses a 4-platform matrix (macOS aarch64/x86_64, Linux aarch64/x86_64) to cross-compile with `cargo build --release`. Linux aarch64 uses `gcc-aarch64-linux-gnu` as a cross-compiler. Each platform produces a `lk-{target}.tar.gz` artifact. The `release` job downloads all artifacts, generates `checksums.txt` with SHA256 hashes, and creates a GitHub Release via `softprops/action-gh-release`.
