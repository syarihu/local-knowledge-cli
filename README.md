...
Commands:
  ...
  setup [agent]     Output agent-specific setup instructions (agy, cursor, claude, codex, all)
...
- **Other AI Agents (Cursor, Antigravity, Codex, etc.)**: Connect the `lk-knowledge` MCP server. Run `lk setup <agent>` (e.g. `lk setup agy`, `lk setup cursor`, or `lk setup` to list all targets) to get connection setup prompts for your agent. Once connected, the MCP server provides knowledge guidelines automatically via the MCP `initialize` response (`InitializeResult.instructions`), so no prompt imports or custom instruction files are needed.
