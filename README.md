...
- **Other AI Agents (Cursor, Antigravity, Codex, etc.)**: Connect the `lk-knowledge` MCP server. Run `lk setup <agent>` (e.g. `lk setup agy`, `lk setup cursor`, or `lk setup` to list all targets) to get tailored instructions for your agent. The server returns instructions automatically via the MCP `initialize` response (`InitializeResult.instructions`), so no prompt imports or manual instructions are needed.
